...
def create_config(hyper_parameters):
    ...
    num_samples = 1 if int(len(prod) * 0.4) < 1 else int(len(prod) * 0.4)
...
def evaluate(config, args, test_en, test_de, test_y, criterion, seq_len, path):
    ...
    for j in range(test_en.shape[0]):
        ...
        y_true = test_y[j].to('cpu')
        loss = criterion(y_true, output.to('cpu'))
        test_loss += loss.item()
        mae_loss += mae(y_true, output.to('cpu')).item()
    test_loss = test_loss / test_en.shape[1]
