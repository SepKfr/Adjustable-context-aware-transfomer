...
def main():
    ...
    parser.add_argument("--kernel", type=list, default=[1, 3, 6, 9])
    parser.add_argument("--filter_length", type=int, default=18)
    ...
    parser.add_argument("--attn_type", type=str, default='context_aware_simple_avg')
    ...
    for i, conf in enumerate(configs, config_num):
        ...
        model = Attn(src_input_size=train_en_p.shape[3],
                     ...
                     tgt_pad_index=0, device=device,
                     attn_type=args.attn_type,
                     kernel=kernel, filter_length=args.filter_length)
        model.to(device)
