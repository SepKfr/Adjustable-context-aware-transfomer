...
def main():
    ...
    y_true = pickle.load(open('y_true_{}.pkl'.format(args.exp_name), 'rb')).cpu().detach()
    print("read y_true")
    y_true_input = pickle.load(open('y_true_input_{}.pkl'.format(args.exp_name), 'rb')).cpu().detach()
    ...
    lstm = pickle.load(open(os.path.join('preds_{}_24'.format(args.exp_name),
                                         'lstm_{}'.format(args.seed)), 'rb')).cpu().detach()
    print("read lstm")
    ...
    attn = pickle.load(open(os.path.join('preds_{}_24'.format(args.exp_name),
                                         'attn_{}'.format(args.seed)), 'rb')).cpu().detach()
    print("read attn")
    ...
    attn_temp_cutoff = pickle.load(open(os.path.join('preds_{}_24'.format(args.exp_name), 'attn_temp_cutoff_{}'
                                                     .format(args.seed)), 'rb')).cpu().detach()
