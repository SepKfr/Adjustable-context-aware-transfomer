...
def perform_evaluation(args, device, params, test, valid_max, formatter):
    ...
    def create_rmse_plot():
        ...
        plt.plot(x_2, np.append(attn_temp_cutoff_48[0::8], attn_temp_cutoff_48[-1]), marker="h", linestyle="-", color='orange')
        plt.plot(x_2, np.append(attn_conv_48[0::8], attn_conv_48[-1]), marker="h", linestyle="-", color='seagreen')
        plt.plot(x_2, np.append(attn_48[0::8], attn_48[-1]), marker="h", linestyle="-", color='red')
        plt.plot(x_2, np.append(attn_multi_48[0::8], attn_multi_48[-1]), marker="h", linestyle="-", color='violet')
        plt.plot(x_2, np.append(lstm_48[0::8], lstm_48[-1]), marker="h", linestyle="-", color='salmon')
        plt.xlabel("Output Length")
        plt.ylabel("RMSE Score")
        plt.legend(['Ours', 'CNN-Trans', 'Transformer', 'Trans-Multi', 'LSTM'], loc="best")
        plt.savefig(os.path.join(args.path_to_save, 'rmses_{}.png'.format(args.exp_name)))
        plt.close()
