...
class Attn(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs, training=True):
        ...
        lstm_enc_inputs = enc_inputs.permute(1, 0, 2)
        lstm_dec_inputs = dec_inputs.permute(1, 0, 2)

        hidden = torch.zeros(self.n_layers, lstm_enc_inputs.shape[1], self.d_model).to(self.device)

        lstm_enc_inputs, _ = self.lstm(lstm_enc_inputs, (hidden, hidden))
        lstm_dec_inputs, _ = self.lstm(lstm_dec_inputs, (hidden, hidden))

        enc_inputs = enc_inputs + lstm_enc_inputs.permute(1, 0, 2)
        dec_inputs = dec_inputs + lstm_dec_inputs.permute(1, 0, 2)
